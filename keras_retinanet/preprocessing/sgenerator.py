...
from ..utils.center_alpha_anchors import (
    anchor_targets_center_alpha,
    anchors_for_shape,
    guess_shapes
)
...
class Generator(keras.utils.Sequence):
    ...
    def __init__(
        ...
        compute_anchor_targets=anchor_targets_center_alpha,
    ...
    def load_annotations_group(self, group):
        ...
        for annotations in annotations_group:
            ...
            assert('labels' in annotations), '\'load_annotations\' should return a list of dictionaries that contain \'labels\' and \'center_alpha\'.'
            assert('center_alpha' in annotations), '\'load_annotations\' should return a list of dictionaries that contain \'labels\' and \'center_alpha\'.'
    ...
    def filter_annotations(self, image_group, annotations_group, group):
        ...
        for index, (image, annotations) in enumerate(zip(image_group, annotations_group)):
            ...
            invalid_indices = np.where(
                (annotations['center_alpha'][:, 2] <= annotations['center_alpha'][:, 0]) |
                (annotations['center_alpha'][:, 3] <= annotations['center_alpha'][:, 1]) |
                (annotations['center_alpha'][:, 0] < 0) |
                (annotations['center_alpha'][:, 1] < 0) |
                (annotations['center_alpha'][:, 2] > image.shape[1]) |
                (annotations['center_alpha'][:, 3] > image.shape[0])
            )[0]
    ...
    def random_transform_group_entry(self, image, annotations, transform=None):
        ...
        if transform is not None or self.transform_generator:
            ...
            annotations['center_alpha'] = annotations['center_alpha'].copy()
            for index in range(annotations['center_alpha'].shape[0]):
                annotations['center_alpha'][index, :] = transform_aabb(transform, annotations['center_alpha'][index, :])
