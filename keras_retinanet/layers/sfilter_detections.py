...
def filter_detections(
    ...
    def _filter_detections(scores, labels):
        ...
        if nms:
            filtered_lines  = tensorflow.gather_nd(lines, indices)
    ...
    lines               = keras.backend.gather(lines, indices)
    labels              = keras.backend.gather(labels, top_indices)
    other_              = [keras.backend.gather(o, indices) for o in other]

    # zero pad the outputs
    pad_size = keras.backend.maximum(0, max_detections - keras.backend.shape(scores)[0])
    lines    = tensorflow.pad(lines, [[0, pad_size], [0, 0]], constant_values=-1)
    ...
    return [lines, scores, labels] + other_
...
class FilterDetections(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        shapes = [(self.max_detections, 3), (self.max_detections,), (self.max_detections,)]
        shapes.extend([(self.max_detections,) + o.shape[2:] for o in other])
        outputs = backend.map_fn(
            _filter_detections,
            elems=[lines, classification, other],
            dtype=dtypes,
            shapes=shapes,
    ...
    def compute_output_shape(self, input_shape):
        ...
        """
        return [
            (input_shape[0][0], self.max_detections, 3),
