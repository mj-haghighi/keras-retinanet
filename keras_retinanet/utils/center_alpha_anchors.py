...
class AnchorParameters:
    ...
    """
    def __init__(self, strides, alpha_segments):
        self.alpha_segments = alpha_segments
        self.strides = strides

    def num_anchors(self):
        return len(self.alpha_segments)
...
AnchorParameters.default = AnchorParameters(
    alpha_segments=get_alpha_segments(),
    strides=[8]
)


def anchor_targets_center_alpha(
    anchors,
    image_group,
    annotations_group,
    num_classes,
    ...
    Args
        anchors: np.array of generated anchors of shape (N, 3) for (x, y, alpha).
        image_group: List of BGR images.
        annotations_group: List of annotation dictionaries with each annotation containing 'labels' and 'center_alphas' of an image.
    ...
    for annotations in annotations_group:
        assert('center_alpha' in annotations), "Annotations should contain center_alpha."
        assert('labels' in annotations), "Annotations should contain labels."

    batch_size = len(image_group)

    regression_batch  = np.zeros((batch_size, anchors.shape[0], 3 + 1), dtype=keras.backend.floatx())
    labels_batch      = np.zeros((batch_size, anchors.shape[0], num_classes + 1), dtype=keras.backend.floatx())

    # compute labels and regression targets
    for index, (image, annotations) in enumerate(zip(image_group, annotations_group)):
        if annotations['center_alpha'].shape[0]:
            # obtain indices of gt annotations with the greatest overlap
            positive_indices, ignore_indices, argmax_overlaps_inds = compute_gt_annotations(anchors, annotations['center_alpha'])
            ...
            regression_batch[index, :, :-1] = center_alpha_transform(anchors, annotations['center_alpha'][argmax_overlaps_inds, :])

        # ignore annotations outside of image
        if image.shape:
            anchors_centers = np.vstack([anchors[:, 0], anchors[:, 1]]).T
...
def compute_overlap(anchors, annotations, max_d = 5, max_d_alpha = 22.5):
    ...
    for anchor, n in enumerate(anchors):
        ...
        for anot, k in enumerate(annotations):
            ...
            if d < max_d and d_alpha < max_d_alpha: # max_d_alpha < 0.07
                overlaps[n, k] = 1
            else:
                if max_d_alpha < d_alpha < max_d_alpha_in_doubt:
                    overlaps[n, k] = 0.5
                if max_d < d < max_d*2:
                    overlaps[n, k] = 0.5
...
def guess_shapes(image_shape, fraction_factor = 8):
    """Guess shapes based on pyramid levels.

    Args
         image_shape: The shape of the image.
         fraction_factor: 2 ** number of Maxpooling2D layers for MaxPooling2D with size=(2,2) and stride=2
    Returns
        A list of image shapes at each pyramid level.
    """
    image_shape = np.array(image_shape[:2])
    image_shapes = [image_shape // fraction_factor]
    return image_shapes
...
def anchors_for_shape(
    ...
    all_anchors = np.zeros((0, 3))
    for idx, shape in enumerate(image_shapes): # we have only one shape
        anchors = generate_anchors() # generate base anchors 
        shifted_anchors = shift(shape, anchor_params.strides[idx], anchors) # shift base anchors over all image
...
def shift(shape, stride, anchors):
    ...
    A = anchors.shape[0]
    K = shifts.shape[0]
    all_anchors = (anchors.reshape((1, A, 3)) + shifts.reshape((1, K, 3)).transpose((1, 0, 2)))
    all_anchors = all_anchors.reshape((K * A, 3))

    return all_anchors


def generate_anchors(alpha_segments=None):
    """
...
def center_alpha_transform(anchors, gt_center_alpha, mean=None, std=None):
    """Compute bounding-box regression targets for an image."""
    ...
    # the following way of bounding box parametrization.
    # See https://github.com/fizyr/keras-retinanet/issues/1273#issuecomment-585828825 for more details
    targets_dx = (gt_center_alpha[:, 0] - anchors[:, 0]) 
    targets_dy = (gt_center_alpha[:, 1] - anchors[:, 1]) 
    targets_dalpha = (gt_center_alpha[:, 2] - anchors[:, 2]) 

    targets = np.stack((targets_dx, targets_dy, targets_dalpha))
    targets = targets.T
